...
kubectl apply -f k8s
kubectl set image deployments/client-deployment server=sujono91/multi-client:$SHA
kubectl set image deployments/server-deployment server=sujono91/multi-server:$SHA
kubectl set image deployments/worker-deployment server=sujono91/multi-worker:$SHA
